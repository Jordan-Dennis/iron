...
\begin{document}
    \section*{1D Ising Model}
        \subsection*{Question 1 a) - Equilibrium at Different Temperatures}
            % So we selected three different temperatures and for each 
            % system we initialised it in a random configuration. OK 
            % so I need to include the metropolis_step_ising_1d code in 
            % this part using the listings package. I also need to make 
            % sure that I talk about the amount of time that it takes 
            % for the system to reach equilibrium. I could back this up 
            % with plots of the physical parameters at each point in 
            % time but I think that I will just come at it from a 
            % qualitative point of view. It will be good to say that 
            % you can do this to check the equilibration times. 
