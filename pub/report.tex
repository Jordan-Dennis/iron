...
field have no tendency to align \cite{Schroeder2021}. It follows %
...
\textbf{Note}: We were given that the critical temperature was %
\(2.27 \epsilon / k\).
...
coherently. I will use \emph{coherent motion} to refer %
...
\emph{coherent domains} have a \emph{quasi-stable} lifetime before decaying. %
...
temperature is that \emph{domains} die at their corners. This %
...
\textbf{Note}: A lot of my discussion in this section is similar %
to the discussion the previous section. They key difference is %
that this discussion contains to a continuous temperature scale %
instead of a discrete. The key result is in the first paragraph %
and if you are satisfied you need not read any further. My %
implementation of the \verb!metropolis_step! is in the final %
segment.


Smoothly varying the temperature I found that the \emph{speckling} %
...
are very small and isolated with out a lifetime. The transition is %
not smooth with respect to \emph{domains}
...
\begin{figure}
    ...
    \caption{The physical parameters of the system as a function of %
        temperature. We observe that the differences are quite %
        small. This is because the averaging is over a very vast %
        number of samples and the lattice sizes are not that different.}
    \label{fig:7}
\end{figure}
...
parameters changed the fastest. This simulation to nine %
hours to run and has been averaged over a vast number of %
runs. 


\textbf{Note}: You may notice that the different sizes are not %
very different. This is because the lattice sizes that I used %
were \(N = 10, 15\) and \(20\). I ran a simulation for nine %
hours at larger sizes but the heat capacity diverged at %
lower temperatures. I did not have time to re-run this simulation %
so I ran it with smaller values.
...
I cannot find an error in my code. At %
...
and industrial interest \cite{Schroeder2021}. For example, magnetic %
...
possible to construct systems that are magnetic without external %
...
was changed by the external magnetic field. To test our predictions for %
the paramagnetic substance we simulated it %
...
we can see that there is an analogue to the \emph{meta-stable} states %
we encountered in the ferro-magnetic case. That is a ring that has formed. %
...
\begin{figure}[h]
    ...
    \caption{Snapshots of the paramagnetic material equilibrated at %
        a variety of temperatures and magnetic fields.}
    \label{fig:9}
\end{figure}
...
Using equations \ref{eqn:9}, \ref{eqn:10}, \ref{eqn:11}, \ref{eqn:12} and %
\ref{eqn:13} we computed \(\chi^{2}\) statistics for the fitted model. %
...
predicted in equations \ref{eqn:10}, \ref{eqn:11}, \ref{eqn:12} and %
\ref{eqn:13}. You may be suspicious of the \(NAN\), \(\chi^{2}\) values %
...
equations \ref{eqn:10}, \ref{eqn:11}, \ref{eqn:12} and \ref{eqn:13}. %
